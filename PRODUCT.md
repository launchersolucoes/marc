...
- Proprietários podem baixar uma cópia completa e estruturada dos dados do estabelecimento. Usuários autenticados e clientes com link ativo registram solicitações de exclusão no próprio produto; a correção permanece disponível nos formulários de perfil e na área do cliente. O proprietário pode encerrar o estabelecimento após confirmação explícita: horários futuros e acessos são desativados, contatos de clientes e profissionais são anonimizados e o histórico financeiro/auditoria necessário é preservado. Assinaturas externas ativas precisam ser canceladas antes do encerramento.
- Administradores da Launcher acompanham solicitações de privacidade em uma fila restrita, registram análise, conclusão ou recusa com justificativa e preservam o histórico de cada decisão.
- O painel Master apresenta uma matriz operacional de retenção por categoria e deixa explícito que os prazos propostos ainda dependem de validação jurídica e contábil. Nenhuma exclusão por prazo é automática; bloqueios por obrigação, disputa ou incidente precisam ser documentados.
- Termos e Política de Privacidade possuem versões correntes e hash SHA-256 do conteúdo registrados no banco. Donos aceitam atomicamente durante o onboarding, convidados antes de entrar na equipe e contas existentes podem renovar o aceite em Configurações; a evidência preserva estabelecimento, origem, versão e horário, além de uma referência pseudonimizada após a exclusão da conta.
...
- Cada confirmação pública agora entrega uma área do cliente sem senha por link seguro, revogável e válido por 90 dias. Nela, o cliente consulta próximos horários e histórico, cancela ou reagenda, acompanha e abandona a lista de espera, atualiza os próprios dados e agenda novamente com o último serviço e profissional pré-selecionados. O banco guarda somente o hash do token; a equipe consulta o estado do acesso, gera, substitui ou revoga o link pela ficha do cliente. As mutações são auditadas sem contatos pessoais nos metadados e protegidas por limite dedicado no banco.
