...
The authenticated PWA must behave like an iOS app on phones: keep the app shell and tab bar persistent across navigation, use route-level skeletons instead of full-screen reload states, present focused creation flows as dismissible sheets with backdrop, handle, Cancel/Done semantics, Escape and swipe-down dismissal, and use 150–250 ms state-driven transitions. Navigation, loading, overlays, safe areas, touch targets, and feedback should follow familiar iOS conventions while preserving Marc branding and responsive desktop behavior.
